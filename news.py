...
for key,values in urls.items():
    ...
    with open(f"e:/react/aa-news/json/{key}.json","w") as f:
        f.write(json_string)
...
while True:
    print(urls.keys())
    print("q for quit")
    selection=input("please enter news name to see rss feed:\n")
    if selection=="q":
        break
    else:
        url=urls[selection]
        os.system("cls")
        feed=get_feed(url)
        if url=="https://feeds.feedburner.com/daily-express-world-news":
            for item in feed:
                item["content"]=item["content"].split(">")[-1]
